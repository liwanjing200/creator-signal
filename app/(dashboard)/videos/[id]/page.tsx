...
import { queueComments, queueTranscription, toggleKeepOriginal } from "@/app/actions";
  ...
  return <>
    ...
    <section className="content-grid">
      <div style={{ display: "grid", gap: 18 }}>
        <div className="card"><div className="card-header"><h2 className="card-title">字幕</h2>{transcript && <Badge tone={transcript.status === "completed" ? "green" : transcript.status === "failed" ? "red" : "neutral"}>{transcript.status}</Badge>}</div><div className="card-body">
          {transcript ? <><dl className="definition-list"><dt>来源</dt><dd>{transcript.source}</dd><dt>语言</dt><dd>{transcript.language ?? "—"}</dd><dt>模型</dt><dd>{transcript.model_name ?? "非模型字幕"}</dd><dt>完成时间</dt><dd>{formatDate(transcript.completed_at, true)}</dd>{transcript.error_message && <><dt>错误</dt><dd style={{ color: "var(--red)" }}>{transcript.error_message}</dd></>}</dl>
            <h3 className="card-title section-gap">清洗字幕</h3><p className="detail-description section-gap" style={{ whiteSpace: "pre-wrap" }}>{transcript.cleaned_text || "暂无清洗文本"}</p><h3 className="card-title section-gap">原始字幕</h3><p className="detail-description section-gap" style={{ whiteSpace: "pre-wrap" }}>{transcript.raw_text || "暂无原始文本"}</p></> : <EmptyState title="暂无字幕" description="点击上方转写按钮加入队列；Mac 会优先读取平台字幕，没有时才临时转写。" />}
        </div></div>
        ...
        <div className="card"><div className="card-header"><h2 className="card-title">评论</h2><div className="top-actions"><span className="cell-subtitle">{comments.length} 条代表性片段</span>{video.platform === "bilibili" && <form action={queueComments.bind(null, id)}><button className="button button-secondary" type="submit">采集代表性评论</button></form>}</div></div><div className="card-body">
      ...
      <aside style={{ display: "grid", gap: 18, alignContent: "start" }}>
        ...
        <div className="card"><div className="card-header"><h2 className="card-title">内容参考</h2><Badge>本地抽取</Badge></div><div className="card-body"><h3 className="card-title">参考摘要</h3><p className="detail-description section-gap">{video.reference_summary || "暂无。完成字幕后会基于章节或字幕做轻量抽取，不冒充深度 AI 分析。"}</p><h3 className="card-title section-gap">候选关键句</h3>{video.candidate_quotes?.length ? <ul>{video.candidate_quotes.map((quote, i) => <li key={i}>{quote}</li>)}</ul> : <p className="page-subtitle section-gap">暂无候选关键句。</p>}</div></div>
        <div className="card"><div className="card-header"><h2 className="card-title">互动数据快照</h2></div><div className="card-body">{metricSnapshots.length ? <div className="timeline">{metricSnapshots.map((snapshot) => <div className="timeline-item" key={snapshot.id}><span className="timeline-dot" /><div className="timeline-title">{formatNumber(snapshot.view_count)} 播放 · {formatNumber(snapshot.like_count)} 赞 · {formatNumber(snapshot.comment_count)} 评论</div><div className="timeline-meta">{formatDate(snapshot.captured_at, true)}</div></div>)}</div> : <p className="page-subtitle">暂无互动快照。每次采集都会追加一条。</p>}</div></div>
